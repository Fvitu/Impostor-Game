...
export function RoleRevealPhase() {
  ...
  const Header = (
		<GameNavbar
			...
			subtitle={"Round " + game.currentRound}
		...
		<div className="min-h-dvh flex flex-col">
			...
			<div className="flex-1 flex items-center justify-center px-4 overflow-hidden">
				<div className="w-full max-w-sm mx-auto text-center flex flex-col justify-center gap-4 py-4 animate-page-enter">
					...
					<p className="text-sm text-muted-foreground mb-4">Pass the device to this player</p>
					...
					<div
						...
						<div style={{ display: isRevealed ? "block" : "none" }}>
							<p className="text-xs font-mono tracking-widest uppercase mb-3 text-muted-foreground">Your Role</p>
							<p className={`text-2xl font-bold mb-4 ${isImpostor ? "text-primary" : "text-success"}`}>
								{isImpostor ? "The Impostor" : "Friend"}
							</p>
							{isImpostor ? (
								<div>
									<p className="text-sm text-muted-foreground mb-2">You do NOT know the secret word.</p>
									{game.impostorHelp && (
									...
									{!game.impostorHelp && <p className="text-xs text-muted-foreground italic">No hints enabled for this game.</p>}
								</div>
							) : (
								<div>
						...
						<div style={{ display: isRevealed ? "none" : "block" }}>
							...
							<p className="text-sm text-muted-foreground">Hold the button below to see your role</p>
						</div>
					</div>
