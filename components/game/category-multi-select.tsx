...
export function CategoryMultiSelect({
	...
	const selectedIds: Set<GameCategoryId> = isAll
		...
		<Popover open={open} onOpenChange={setOpen}>
			<PopoverTrigger asChild disabled={disabled}>
				<button
					...
					className="h-12 w-full flex items-center justify-between rounded-md border border-border bg-secondary/40 px-3 text-sm text-foreground hover:bg-secondary/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/35 data-[state=open]:border-primary/40 transition-[color,border-color]"
				>
			...
			>
				...
				<div className="p-2 pb-0">
					<label className="flex items-center gap-3 rounded-md px-2 py-2 hover:bg-primary/10 cursor-pointer select-none">
						<Checkbox
							checked={isAll}
				...
				<div className="overflow-y-auto max-h-52 p-2 pt-1">
					<div className="space-y-0.5">
						{options.map((option) => (
							<label
								key={option.id}
								className="flex items-center gap-3 rounded-md px-2 py-2 hover:bg-primary/10 cursor-pointer select-none"
							>
								<Checkbox
